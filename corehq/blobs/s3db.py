...
class S3BlobDB(AbstractBlobDB):
    ...
    def report_timing(self, action, path):
    ...
    def put(self, content, **blob_meta_args):
        ...
        check_safe_path(meta.path)
        s3_bucket = self._s3_bucket(create=True)
        if isinstance(content, BlobStream) and content.blob_db is self:
            obj = s3_bucket.Object(content.blob_path)
            meta.content_length = obj.content_length
            self.metadb.put(meta)
            source = {"Bucket": self.s3_bucket_name, "Key": content.blob_path}
            with self.report_timing('put-via-copy', meta.path):
                s3_bucket.copy(source, meta.path)
        else:
            content.seek(0)
            meta.content_length = get_file_size(content)
            self.metadb.put(meta)
            with self.report_timing('put', meta.path):
                s3_bucket.upload_fileobj(content, meta.path)
        return meta

    def get(self, path):
        check_safe_path(path)
        with maybe_not_found(throw=NotFound(path)), self.report_timing('get', path):
            resp = self._s3_bucket().Object(path).get()
        return BlobStream(resp["Body"], self, path)

    def size(self, path):
        check_safe_path(path)
        with maybe_not_found(throw=NotFound(path)), self.report_timing('size', path):
            return self._s3_bucket().Object(path).content_length

    def exists(self, path):
        check_safe_path(path)
        try:
            with maybe_not_found(throw=NotFound(path)), self.report_timing('exists', path):
    ...
    def delete(self, path):
        ...
        with maybe_not_found():
            ...
            obj.delete()
            success = True
        self.metadb.delete(path, deleted_bytes)
        return success

    def bulk_delete(self, metas):
        success = True
        for chunk in chunked(metas, 1000):
            objects = [{"Key": meta.path} for meta in chunk]
            s3_bucket = self._s3_bucket()
