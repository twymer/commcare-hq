...
class FormAccessorSQL(AbstractFormAccessor):
    ...
    def get_attachment_content(form_id, attachment_name, stream=False):
        ...
        return AttachmentContent(meta.content_type, get_blob_db().get(meta.path))
    ...
    def get_forms_with_attachments_meta(form_ids, ordered=False):
        ...
        attachments = self.get_attachments_for_forms(form_ids)
    ...
    def hard_delete_forms(domain, form_ids, delete_attachments=True):
        ...
        if delete_attachments:
            attachments = FormAccessorSQL.get_attachments_for_forms(form_ids)
        ...
        if delete_attachments:
            ...
            if deleted_count != len(form_ids):
                ...
                attachments_to_delete = [
                    ...
                    if attachment.parent_id in deleted_forms
                ]

            get_blob_db().bulk_delete(attachments_to_delete)

        return deleted_count

    @staticmethod
    def get_attachments_for_forms(form_ids):
        assert isinstance(form_ids, list)
        if not form_ids:
            return []
            ...
            for unsaved_attachment in unsaved_attachments:
                if unsaved_attachment.is_saved():
                    raise XFormSaveError(
                        'attachment {} has already been saved'.format(unsaved_attachment.name)
                    )
