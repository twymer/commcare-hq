...
BLOB_MIXIN_MODELS = {
    ...
    CODES.form: xform.XFormInstance,
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            help="Log files with blobs to check. Each line of the log file "
                 ...
                 "type_code, parent_id, blob_path, and error fields",
        )
        parser.add_argument(
            '--migrate',
            action="store_true",
...
def check_blob(doc_type, rec, old_db, new_db, migrate=False):
    ...
    if new_db.exists(path):
        return "new"

    if old_db.exists(path):
        if migrate:
            with old_db.get(path) as content:
                new_db.copy_blob(content, path)
            migrated = " migrated"
        else:
            migrated = ""
    ...
    if doc_type is not None:
        try:
            doc = doc_type.get_db().get(rec["parent_id"])
